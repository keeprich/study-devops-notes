...
Rolling update: is whe you update your runing servers one at a time to ensure users always have access to the site

Deployment definition files are similar to replicaset. and when ever a deployment runs it can automatically create a replicaset
...
====================== SERVICES AND EXAMS TIPS ======================


It helps us to connects other apps together andalso helpus togive access of oursite to users
...
= ClusterIP: this provides acces to the pods inside the node. it makes it possible for pods tocommunicate amonge themselfs.
...
============ to create the seervice, run 'kubectl create -f service-definition.yml ===



===== NAMESPACES ==============

Allow for seperation of groups into different unit which specified resourcerequirments and permissions
...
kubectl create namespace namefornamespace
	
	OR
	
kubectl create --namesapce=namespacename filename.yml
	
=== To switch tocreated namespace and work there ---------- run
	
	kubectl config set-context ${kubectl config current-context} --namespace=namespacetoswitch
	
=== To view pod in all namespaces ----------- run
